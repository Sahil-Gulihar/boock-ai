...
6. **"run the full suite before committing each task"** — after every implementation step,
   ...
   commit.
7. Post-submission follow-up: **"the system is small and output quality matters the most
   here, can u switch out minimax to openai and use that instead?"** — required re-fetching
   OpenAI's current image API docs (`developers.openai.com/api/docs/guides/image-generation`)
...
is 30 passing tests covering all 4 required categories (reference conditioning contract,
mixed-family hard block, LangGraph smoke test, DynamoDB persistence) plus bonus categories
(repair tickets, FastAPI, Lambda handler, image metadata, OpenAI provider, provider
factory). The sample run in
