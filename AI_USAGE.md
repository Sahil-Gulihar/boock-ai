...
9. **A request to use provided images as generation input** that required pushing back, not
   complying directly: one image was a real, identifiable person's photo offered as a face
   reference for Mira, and two were photos of an actual seal offered as the "Black Seal"
   prop reference. Declined the first outright — the assignment's own hard rule prohibits
   imitating a real living person's likeness, and that doesn't change because the request
   came from the user rather than the input files. Flagged the second as counterproductive
   rather than unsafe: feeding a real seal animal photo as the reference for a fictional
   obsidian talisman would reinforce the exact literal-name misreading already documented
   as a bug, not fix it. Offered a safe alternative (search for a properly licensed seal
   photo to use only as illustrative comparison in docs, never as a generation input) and a
   real fix (root-cause and correct the actual prompt bug), and the user chose both.
...
## Root-causing the prop-drift bug instead of covering it up

`EXAMPLES.md` already documented, honestly, that a real provider run had rendered the
`black_seal` prop as a literal animal and that QA had passed it anyway. The obvious lazy
fix — just add a "not an animal" phrase to `provided_inputs/scene_packets.json`'s
`prompt_intent` — was rejected as the wrong layer: that file is Boock-supplied input data,
not something this pipeline should be quietly hand-editing to make one demo scene look
better. The actual bug was one level down: `select_generation_strategy` built the render
prompt from `scene.prompt_intent` alone, silently discarding the `preserve_facets` metadata
the reference conditioning contract had *already collected* for every entity. The fix
(`_grounded_prompt()`) uses that existing metadata generically, for every character and
prop in every scene, not a special case for `black_seal`. Verified by re-running the exact
same scene against the real provider before and after the fix and comparing the actual
generated images — not just asserting the prompt string contains the right substrings.
...
run the *entire* suite (not just the new file), then commit. The final state is 46 passing
...
persistence factory, family drift, memory coverage, prompt grounding). The sample run in
